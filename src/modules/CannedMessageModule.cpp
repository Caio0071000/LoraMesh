...
{
    // Log das partes principais do pacote
    LOG_DEBUG("Received message details:");
    LOG_DEBUG("  Port Number: %d", mp.decoded.portnum);
    LOG_DEBUG("  Request ID: %d", mp.decoded.request_id);
    LOG_DEBUG("  From Node: %d", mp.from);

    // Se o portnum for RoutingApp, tenta decodificar e logar o conteúdo
    if (mp.decoded.portnum == meshtastic_PortNum_ROUTING_APP) {
        ...
        if (pb_decode_from_bytes(mp.decoded.payload.bytes, mp.decoded.payload.size, meshtastic_Routing_fields, &decoded)) {
            LOG_DEBUG("  Routing Error Reason: %d", decoded.error_reason);
        } else {
            LOG_ERROR("  Failed to decode Routing payload.");
        }
    }

    // Imprime os bytes do payload
    LOG_DEBUG("  Payload (hex):");
    for (int i = 0; i < mp.decoded.payload.size; i++) {
        LOG_DEBUG("    0x%02x ", mp.decoded.payload.bytes[i]);
        if ((i + 1) % 16 == 0) {
            LOG_DEBUG("");  // Nova linha a cada 16 bytes
        }
    }
